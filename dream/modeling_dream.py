# This is reference code. Do not make any changes to this file.
...
from .generation_utils_cached_pd import DreamGenerationMixinWithCache, DreamGenerationConfig
...
class MaskedLMOutputWithPastKeyValues(MaskedLMOutput):
    def __init__(self, loss=None, logits=None, hidden_states=None, attentions=None, past_key_values: Optional[List[Tuple[torch.Tensor, torch.Tensor]]] = None):
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1, replace_index: Optional[torch.Tensor] = None):
    ...
    # print("slice range would be:", key_len - query_len, ":", key_len)
    ...
    if replace_index is None:
        # print("cos.shape inside", cos[:, :, key_len - query_len : key_len, :].shape)
        # if query_len > 1:
        #     q_embed = (q * cos[:, :, key_len - query_len : key_len, :]) + (rotate_half(q) * sin[:, :, key_len - query_len : key_len, :])
        # else:
        #     q_embed = (q * cos) + (rotate_half(q) * sin)
        ...
        q_embed = (q * cos[:, :, replace_index, :]) + (rotate_half(q) * sin[:, :, replace_index, :])
        
    k_embed = (k * cos) + (rotate_half(k) * sin)
    
    # print("qk", q_embed.shape, k_embed.shape)
...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        if dual_cache:
            ...
            query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, replace_index=replace_indices)
        else:
            query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin)
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def forward(
        ...
        #     print("position_ids 1", position_ids)
        #     print("cache_position 1", cache_position)

        # else:
        #     if position_ids is None:
        #         if past_key_values is not None:
        #             position_ids = torch.arange(past_seen_tokens, device=inputs_embeds.device).unsqueeze(0)
        #             print("position_ids 2", position_ids)
        #             print("cache_position 2", cache_position)
        #         else:
        #             position_ids = torch.arange(inputs_embeds.shape[1], device=inputs_embeds.device).unsqueeze(0)
        #             print("position_ids 3", position_ids)
        #             print("cache_position 3", cache_position)
...
class DreamModel(DreamGenerationMixin, DreamGenerationMixinWithCache, DreamPreTrainedModel):
    """
    DreamModel that can use either DreamGenerationMixin or DreamGenerationMixinWithCache
    based on the use_cache configuration parameter.
    """
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        # Determine which mixin to use
        self.use_cache = getattr(config, 'use_cache', False)
        
        # Initialize with the appropriate parent class
        if self.use_cache:
            # Call DreamGenerationMixinWithCache methods
            DreamGenerationMixinWithCache.__init__(self)
        else:
            # Call DreamGenerationMixin methods  
            DreamGenerationMixin.__init__(self)
            
        DreamPreTrainedModel.__init__(self, config)
    ...
    def __getattribute__(self, name):
        """Override method resolution to use the appropriate mixin based on use_cache"""
        # For generation methods, use the appropriate mixin
        if name in ['planned_diffusion_generate', 'diffusion_generate', '_diff_sample', '_ar_sample'] and hasattr(self, 'use_cache'):
            if self.use_cache and hasattr(DreamGenerationMixinWithCache, name):
                return getattr(DreamGenerationMixinWithCache, name).__get__(self, type(self))
            elif not self.use_cache and hasattr(DreamGenerationMixin, name):
                return getattr(DreamGenerationMixin, name).__get__(self, type(self))
        
        # Use default attribute access for everything else
