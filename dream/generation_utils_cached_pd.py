...
from .pd_utils import create_pd_inputs, update_attention_mask, invert_and_expand_attention_mask, create_replace_mask, pad_key_values, block_unmask, block_unmask_confidence_threshold
...
@dataclass
class DreamModelOutputWithPast(ModelOutput):
    sequences: torch.LongTensor = None
...
class DreamGenerationMixinWithCache:
    @staticmethod
    ...
    def _ar_sample(
        ...
        while True:
            ...
            if past_key_values is None:
                ...
                inverted_attention_mask = invert_and_expand_attention_mask(curr_attention_mask.narrow(-2, -1, 1), self.dtype)
        ...
        return input_ids, curr_attention_mask, past_key_values
    ...
    def planned_diffusion_generate(
        ...
        **kwargs,
        ...
        while not end_of_generation:
            ...
            curr_input_ids, curr_attention_mask, past_key_values = self._ar_sample(
                curr_input_ids,
                curr_attention_mask,
                past_key_values,
            ...
            if final_planning_input_id.item() == SYNC_TOKEN_ID:
                end_of_generation = False
                curr_input_ids = output.sequences
    ...
    def _diff_sample(
        ...
        if attention_mask is not None:
            ...
            if attention_mask.dim() == 2:
                ...
                tok_idx = torch.arange(attention_mask.shape[2], device=attention_mask.device).unsqueeze(0)
        ...
        logits_placeholder = torch.zeros((1, x.shape[1], self.config.vocab_size), device=x.device, dtype=self.dtype)
        
        # Track completed blocks for KV caching
        ...
        while i < steps:
            mask_index = (x == mask_token_id)
            # Check which blocks are newly completed for KV caching
            if block_info is not None:
                for block_idx, (start_idx, end_idx, _) in enumerate(block_info):
                    block_mask = mask_index[0, start_idx:end_idx]
                    if block_mask.sum() == 0 and block_idx not in completed_blocks:
                        completed_blocks.add(block_idx)
                        
                        
            # Use KV caching when we have completed blocks to reuse
            # if completed_blocks and past_key_values is not None and block_info is not None
            replace_mask = create_replace_mask(completed_blocks, block_info, x.shape[1])
            
            if i == 0: # prefilling step
                past_key_values = pad_key_values(past_key_values, x.shape[1])

            curr_input_ids = x[replace_mask].unsqueeze(0)
            curr_attention_mask = attention_mask[:,:, replace_mask[0], :]

            
            outputs = self(input_ids=curr_input_ids, 
                            attention_mask=curr_attention_mask, 
                            use_cache=True,
                            dual_cache=True,
            ...
            logits_placeholder[replace_mask] = outputs.logits
            logits = logits_placeholder.clone()
            ...
            if alg == 'origin':
                ...
                number_transfer_tokens = mask_index.sum().item() // (og_num_steps - i) if i < og_num_steps else 0 # Add this for non-divisible ste
        ...
        return DreamModelOutputWithPast(
            sequences=x,
            past_key_values=past_key_values
        )
